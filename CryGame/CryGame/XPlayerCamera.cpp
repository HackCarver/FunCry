...
bool Effects::NeedWeapShaderUpdate = false; /// FunCry modification
bool Effects::NeedWeapShaderForce = false; /// FunCry modification
static bool LastWasCIAWork = false; /// FunCry modification
static bool LastWasCyberCry = false; /// FunCry modification
static int LastWeaponID = -1; /// FunCry modification
...
{	
	...
	float ActualSpeed; /// FunCry modification
	if( Effects::IsDrunk ){ /// FunCry modification
    ActualSpeed = m_walkParams.speed;
    m_walkParams.speed = m_pGame->p_speed_run->GetFVal( );
	}
	...
	if( Effects::IsDrunk ){ /// FunCry modification
	  m_walkParams.speed = ActualSpeed;
	}
    ...
    if( Effects::IsFunkyWeapon ){ /// FunCry modification
      weaponOffset.x -= 0.35f;
      m_vWeaponAngles.y = 30;
      m_vWeaponAngles.z = 20;
    }

		pSelectedWeapon->SetFirstPersonOffset( weaponOffset, m_vWeaponAngles);
		pSelectedWeapon->MoveToFirstPersonPos(m_pEntity);

		if( Effects::NeedWeapShaderUpdate ){ /// FunCry modification
      if(
        Effects::NeedWeapShaderForce
        ||
        LastWasCIAWork != Effects::CIAWork
        ||
        LastWasCyberCry != Effects::IsCyberCry
        ||
        ( LastWasCIAWork || LastWasCyberCry ) != ( Effects::CIAWork || Effects::IsCyberCry )
        ||
        (
          LastWeaponID != m_nSelectedWeaponID
          &&
          (
            Effects::CIAWork
            ||
        ...
        LastWasCIAWork = false;
        LastWasCyberCry = false;

        ICryCharInstance *Model = pSelectedWeapon->GetCharacter( );
        if( Model ){
		      if( Effects::CIAWork ){
            Model->SetShaderTemplateName( "" , 0 , 0 , 0 );
            Model->SetShaderTemplateName( "TEMPLCRYVISION_MASK" , 1 );
            LastWasCIAWork = true;
		      }
		      else if( Effects::IsCyberCry ){
            Model->SetShaderTemplateName( "" , 0 );
            Model->SetShaderTemplateName( "TEMPLHOLOGRAM" , 1 );
            LastWasCyberCry = true;
		      }
		      else{
            Model->SetShaderTemplateName( "" , 0 );
            Model->SetShaderTemplateName( "" , 1 );
		      }
		      LastWeaponID = m_nSelectedWeaponID;
		      Effects::NeedWeapShaderForce = false;
        }
      }
      Effects::NeedWeapShaderUpdate = false;
		}
