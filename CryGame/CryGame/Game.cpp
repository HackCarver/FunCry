...
CXGame::~CXGame()
{
  FunCry::Stop( ); /// FunCry modification
...
void CXGame::Reset()
{
  ...
  FunCry::Reset( this ); /// FunCry modification
}
...
{	
  ...
  m_pSystem->GetILog()->Log("Game Initialization");
	...
	FunCry::Start( this ); /// FunCry modification
...
bool CXGame::Update()
{
    ...
    Effects::DXBeforeRender( ); /// FunCry modification

		m_pSystem->Render();

		Effects::DXAfterRender( ); /// FunCry modification

		pTimer->MeasureTime("3SysRend");
	}

  Effects::DoAimedEffect( ); /// FunCry modification
  Effects::DoScreensaver( ); /// FunCry modification
  ...
  FunCry::Update( bPause ); /// FunCry modification
...
{
  FunCry::Update( false ); /// FunCry modification
...
void CXGame::ProcessPMessages(const char *szMsg)
{
    ...
    if(m_bEditor) 
      ...
      FunCry::PauseAll( ); /// FunCry modification
      FunCry::IsPaused = true; /// FunCry modification
	...
	else if (strnicmp(szMsg,"StartLevel",10)==0)		 // start a level
	{
		if (!m_bEditor)
		{
			...
			else
			{
				...
				FunCry::PauseAll( ); /// FunCry modification
				FunCry::IsLoading = true; /// FunCry modification
...
void CXGame::LoadLevelCS(bool keepclient, const char *szMapName, const char *szMissionName, bool listen)
{
  ...
  FunCry::IsLoading = true; /// FunCry modification
	...
	if((!m_pServer || !keepclient) && !StartupServer(listen))
	{
		...
		FunCry::IsLoading = false; /// FunCry modification
