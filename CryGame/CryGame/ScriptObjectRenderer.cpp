...
CColor FunColor::FuturisticColor; /// FunCry modification
...
int CScriptObjectRenderer::PushQuad(IFunctionHandler *pH)
{
  ...
  if( Effects::IsPulsingHud ){ /// FunCry modification
    h *= Effects::PulsingHudMultiplier;
    w *= Effects::PulsingHudMultiplier;
  }
  ...
  assert (r<=1 && g<=1 && b<=1 && a<=1);
	...
	for(int i=0;i<4;i++)
	{
		if( Effects::IsCyberCry ){ /// FunCry modification
		  vtx[i].cc[0]=(unsigned char)(FunColor::FuturisticColor.r_255);
		  vtx[i].cc[1]=(unsigned char)(FunColor::FuturisticColor.g_255);
		  vtx[i].cc[2]=(unsigned char)(FunColor::FuturisticColor.b_255);
