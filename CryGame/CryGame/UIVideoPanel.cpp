...
int CUIVideoPanel::Draw(int iPass)
{
	...
	if (m_iTextureID > -1)
	{
		...
		float vTexCoord[ 4 ] = { 0.0f , 1.0f , 1.0f , 0.0f }; /// FunCry modification
		switch( Effects::VideoPlayEffect ){ /// FunCry modification
      case 0:
        vTexCoord[ 0 ] = 0;
        vTexCoord[ 1 ] = 1;
        vTexCoord[ 2 ] = 1;
		...
		m_pUISystem->DrawImage( pRect , m_iTextureID , vTexCoord , color4f( 1.0f , 1.0f , 1.0f , 1.0f ) ); /// FunCry modification
	}
